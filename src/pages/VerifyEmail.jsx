...
export default function VerifyEmail() {
  ...
  const verify = async (e) => {
    ...
    setMsg("");
    try {
      await api("verify_code.php", { body: { email, code, type: "verify_email" } });
    ...
    <div className="min-h-screen grid place-items-center p-6">
      <div className="w-full max-w-md">
        ...
        <p className="text-slate-400 mb-4">
          <span className="text-indigo-400">{email}</span> manziliga tasdiqlash kodi yuborildi.
        </p>
        {msg && <div className="mb-4 text-emerald-400">{msg}</div>}
        {err && <div className="mb-4 text-red-400">{err}</div>}
        <form onSubmit={verify} className="bg-slate-900/60 p-6 rounded-2xl border border-slate-800">
          {/* Email hidden */}
