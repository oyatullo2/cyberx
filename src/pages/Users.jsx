...
export default function Users() {
  ...
  async function load(p, reset = false) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${new URL("./", location.href)}api`, {
        method: "HEAD",
      }).catch(() => {});
      const data = await api(`users_list.php?page=${p}&per=24`, {
        method: "GET",
        token,
      });
      ...
      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {items.map((u) => {
          ...
          return (
            <div
              key={u.id}
              className="rounded-2xl border border-slate-800 bg-slate-900/50 p-3 hover:border-indigo-600 transition"
            >
