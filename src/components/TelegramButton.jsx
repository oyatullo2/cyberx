...
export default function TelegramButton({ text = "Telegram orqali kirish" }) {
  const { adoptSessionFromTelegram } = useAuth();
  ...
  async function handleClick() {
    ...
    if (!window.Telegram || !window.Telegram.Login) {
      alert(
        "Telegram Login skripti yuklanmadi. Internet yoki CSP ni tekshiring."
      );
      return;
    }
    ...
    window.Telegram.Login.auth(
      ...
      async (response) => {
        ...
        try {
          // 🔥 Siz aytgan qo‘shimcha joy shu yerda:
          await adoptSessionFromTelegram(response);
          nav("/dashboard");
        } catch (e) {
